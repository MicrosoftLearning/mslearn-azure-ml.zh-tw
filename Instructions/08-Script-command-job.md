---
lab:
  title: 在 Azure Machine Learning 中以命令作業的形式執行定型腳本
---

# 在 Azure Machine Learning 中以命令作業的形式執行定型腳本
...
1. 將新檔案命名為 `train-classification-model.py`。 
...
1. 檢閱作業的屬性。 請注意， **Command** 會顯示命令作業的 **命令** 參數值：您執行執行腳本的命令。
