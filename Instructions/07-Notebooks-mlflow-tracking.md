...
您通常會藉由實驗和定型多個模型來啟動新的數據科學專案。 若要追蹤您的工作，並保留您所定型模型的概觀，以及其執行方式，您可以使用 MLflow 追蹤。

在此練習中，您會在計算實例上執行的筆記本內 MLflow 來記錄模型定型。
...
您將使用 Azure CLI 來布建工作區和必要的計算，而您將使用 Python SDK 來使用自動化 機器學習 來定型分類模型。

### 建立工作區和計算資源

若要建立 Azure 機器學習 工作區和計算實例，您將使用 Azure CLI。 所有必要的命令都會分組在殼層腳本中，讓您執行。
...
1. 在 Azure Machine Learning 工作室 中，流覽至 **[計算**] 頁面，並確認您在上一節中建立的計算實例是否存在。 計算實例應該正在執行。
1. 在 [ **計算實例] 索引標籤中** ，尋找您的計算實例，然後選取 [ **終端** 機] 應用程式。
1. 在終端機中，在終端機中執行下列命令，在計算實例上安裝 Python SDK 和 MLflow 連結庫：
...
既然您已擁有所有必要的資源，您可以在筆記本中定型模型時執行筆記本來設定和使用 MLflow。

1. **使用 MLflow.ipynb** 筆記本開啟 Labs/07/Track 模型定型。
