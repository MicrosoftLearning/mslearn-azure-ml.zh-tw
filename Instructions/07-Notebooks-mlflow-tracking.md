---
lab:
  title: 使用 MLflow 在筆記本中追蹤模型定型
---

# 使用 MLflow 在筆記本中追蹤模型定型

您通常會藉由實驗和定型多個模型來開始新的資料科學專案。 若要追蹤您的工作，並持續瞭解您定型的模型及其執行方式，您可以使用 MLflow 追蹤。
...
若要建立 Azure Machine Learning 工作區和計算實例，您將使用 Azure CLI。 所有必要的命令會分組在殼層腳本中，供您執行。
...
1. 在Azure Machine Learning 工作室內，流覽至 [**計算**] 頁面，並確認您在上一節中建立的計算實例是否存在。 計算實例應該正在執行。
1. 在 [ **計算實例] 索引標籤** 中，尋找您的計算實例，然後選取 **[終端機** ] 應用程式。
1. 在終端機中，在終端機中執行下列命令，在計算實例上安裝 Python SDK 和 MLflow 程式庫：
...
現在您已擁有所有必要的資源，您可以在筆記本中定型模型時，執行筆記本來設定及使用 MLflow。
