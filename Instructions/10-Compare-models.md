...
定型模型之後，您會想要評估模型，以探索模型是否如預期般執行。 除了效能計量之外，還有其他因素可以納入考慮。 Azure Machine Learning 中負責任的 AI 儀表板可讓您分析資料和模型的預測，以識別任何偏差或不當性。

在此練習中，您將準備您的資料，並在 Azure Machine Learning 中建立負責任的 AI 儀表板。
