---
lab:
  title: 探索工作區互動的開發人員工具
---

# 探索工作區互動的開發人員工具

您可以使用各種工具來與 Azure Machine Learning 工作區互動。 根據您需要執行的工作，以及開發人員工具的喜好設定，您可以選擇何時要使用的工具。 此實驗室設計為開發人員工具的簡介，常用於工作區互動。 如果您想要深入瞭解如何使用特定工具，還有其他實驗室可供探索。
...
- **Azure CLI** 搭配 Azure Machine Learning 擴充功能：此命令列方法適用于基礎結構的自動化。
- **Azure Machine Learning 工作室**：使用使用者易記的 UI 來探索工作區及其所有功能。
...
1. 請移除第 1 版和第 2 版 (任何 ML CLI 延伸模組) ，以避免與舊版發生任何衝突，並使用此命令：
...
訓練機器學習模型所需的基礎結構的另一個重要部分是 **計算**。 雖然您可以在本機定型模型，但使用雲端計算會更具延展性和成本效益。 

當資料科學家在 Azure Machine Learning 工作區中開發機器學習模型時，他們想要使用可執行 Jupyter Notebook 的虛擬機器。 針對開發， **計算實例** 很適合。 
...
1. 使用下列命令在您的工作區中建立計算實例。 如果計算實例名稱包含 「XXXX」，請將它取代為亂數字，以建立唯一的名稱。 

    ```azurecli
    az ml compute create --name "ciXXXX" --size STANDARD_DS11_V2 --type ComputeInstance -w mlw-dp100-labs -g rg-dp100-labs
    ```

    如果您收到錯誤訊息，指出名稱為的計算實例已經存在，請變更名稱，然後重試命令。

## 使用 Azure CLI 建立計算叢集

雖然計算實例非常適合用於開發，但當我們想要定型機器學習模型時，計算叢集更適合。 只有在提交作業以使用計算叢集時，才會調整為超過 0 個節點並執行作業。 一旦不再需要計算叢集，它就會自動重設回 0 個節點的大小，以將成本降至最低。 
...
## 使用 Azure Machine Learning 工作室 設定您的工作站

雖然 Azure CLI 很適合自動化，但您可能會想要檢閱所執行命令的輸出。 您可以使用 Azure Machine Learning Studio 來檢查是否已建立資源和資產，以及檢查作業是否成功執行，或檢閱作業失敗的原因。 
...
1. 當命令完成時，在 [ **檔案** ] 窗格中，選取 ** [&#8635;** 以重新整理檢視，並確認已建立新的 **Users/*your-user-name*/azure-ml-labs** 資料夾。 
1. 開啟 **Labs/02/執行訓練腳本.ipynb** 筆記本。
    
    > 如果出現通知要求您進行驗證，請選取 [ **驗證** ]，並遵循必要的步驟。 

1. 確認筆記本使用 **Python 3.8 - AzureML** 核心。 每個核心都有自己的映射，並預先安裝一組套件。
1. 執行筆記本中的所有儲存格。 
...
1. 新的實驗會列出名為 **diabetes-training**。 選取最新的 **工作 diabetes-pythonv2-train**。
1. 檢閱**作業的屬性。** 請注意作業 **狀態**： 
    - **已排入佇列**：作業正在等候計算可用。
    - **準備**：計算叢集正在調整大小，或正在計算目標上安裝環境。
    - **執行**中：正在執行定型腳本。 
    - **完成**：定型腳本已執行，且作業會以所有最終資訊進行更新。
    - **已完成**：作業已成功完成並終止。
    - **失敗**：作業失敗且已終止。
1. 當作業完成時，您可以在 [**計量**] 下找到模型的*AUC*和*精確度*。 
1. 在 **[輸出 + 記錄**] 底下，您會發現：
    - 資料夾中的模型選擇檔案 **會輸出**。 
    - user_logs **/std_log.txt**中的腳本輸出。 腳本中 **列印** 語句的輸出會顯示在這裡。 如果因為腳本發生問題而發生錯誤，您也會在這裡找到錯誤訊息。
