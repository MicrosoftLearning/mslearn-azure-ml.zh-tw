...
- **Azure CLI** 搭配 Azure 機器學習 擴充功能：此命令行方法非常適合用於基礎結構自動化。
- **** Azure Machine Learning 工作室：使用方便使用的UI來探索工作區及其所有功能。
- **適用於 Azure 的 Python SDK** 機器學習：用來從 Jupyter Notebook 提交作業和管理模型，適用於數據科學家。

您將探索這些工具中的每一個工具，以取得通常使用此工具完成的工作。

## 使用 Azure CLI 布建基礎結構

若要讓數據科學家使用 Azure 機器學習 定型機器學習模型，您必須設定必要的基礎結構。 您可以使用 Azure CLI 搭配 Azure 機器學習 擴充功能來建立 Azure 機器學習 工作區和資源，例如計算實例。

若要開始，請開啟 Azure Cloud Shell，安裝 Azure 機器學習 擴充功能，並複製 Git 存放庫。
...
1. 拿掉任何 ML CLI 擴充功能（第 1 版和 2 版），以避免與舊版發生任何衝突，並搭配此命令：
...
當數據科學家在 Azure 機器學習 工作區中開發機器學習模型時，他們想要使用可執行 Jupyter Notebook 的虛擬機。 針對開發， **計算實例** 很適合。

建立 Azure 機器學習 工作區之後，您也可以使用 Azure CLI 建立計算實例。
...
- **Azure 機器學習 工作區名稱**：mlw-dp100-labs
- **資源群組**：rg-dp100-labs

1. 使用下列命令在工作區中建立計算實例。 如果計算實例名稱包含 「XXXX」 請將它取代為隨機數位，以建立唯一的名稱。
...
雖然計算實例很適合用於開發，但我們想要定型機器學習模型時，計算叢集更適合。 只有在提交作業以使用計算叢集時，才會調整為0個以上的節點並執行作業。 不再需要計算叢集之後，其會自動重設大小回 0 個節點，以將成本降到最低。 

若要建立計算叢集，您可以使用 Azure CLI，類似於建立計算實例。
...
雖然 Azure CLI 很適合自動化，但您可能會想要檢閱您所執行命令的輸出。 您可以使用 Azure Machine Learning 工作室 來檢查是否已建立資源和資產，以及檢查作業是否成功執行，或檢閱作業失敗的原因。 
...
1. 確認筆記本使用 **Python 3.8 - AzureML** 核心。 每個核心都有自己的映像，並預安裝自己的套件集。
1. 在筆記本中執行所有儲存格。

將在 Azure 機器學習 工作區中建立新的作業。 作業會追蹤作業組態中定義的輸入、所使用的程式代碼，以及用來評估模型的計量等輸出。

## 在 Azure Machine Learning 工作室 中檢閱您的作業歷程記錄

當您將作業提交至 Azure 機器學習 工作區時，您可以在 Azure Machine Learning 工作室 中檢閱其狀態。

1. 選取筆記本中提供做為輸出的作業 URL，或流覽至 **Azure Machine Learning 工作室 中的 [作業**] 頁面。
1. 新的實驗被列為 **糖尿病訓練**。 選取最新的工作 **diabetes-pythonv2-train**。
1. 檢閱作業的 **[屬性**]。 請注意工作 **狀態**：
    - **** 已排入佇列：作業正在等候計算可供使用。
    - **準備**：計算叢集正在重設大小，或正在計算目標上安裝環境。
    - **執行**中：正在執行定型腳本。
    - **** 完成：執行定型腳本，且作業會以所有最終資訊進行更新。
    - **已完成**：作業已順利完成，且已終止。
    - **失敗**：作業失敗，且已終止。
1. 在 [輸出 + 記錄] 下 **，您會在 user_logs/std_log.txt** 中找到**腳本的**輸出。 **文稿中列印**語句的輸出會顯示在這裡。 如果因為文稿發生問題而發生錯誤，您也會在這裡找到錯誤訊息。
1. 在 **[程序代碼**] 底下，您會發現您在作業組態中指定的資料夾。 此資料夾包含定型文稿和數據集。
