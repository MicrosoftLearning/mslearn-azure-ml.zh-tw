...
若要開始，請開啟 Azure Cloud Shell，安裝 Azure 機器學習 擴充功能並複製 Git 存放庫。
...
新的作業將會在 Azure 機器學習 工作區中建立。 作業會追蹤作業組態中定義的輸入、所使用的程式代碼，以及用來評估模型的計量等輸出。
...
1. 檢閱作業的 **[屬性**]。 請注意工作 **狀態**：
    ...
    - **已完成**：作業已順利完成，且已終止。
    - **失敗**：作業失敗，且已終止。
1. 在 **[輸出 + 記錄**] 下，您會在 user_logs/std_log.txt** 中找到**腳本的輸出。 **文稿中列印**語句的輸出會顯示在這裡。 如果因為文稿發生問題而發生錯誤，您也會在這裡找到錯誤訊息。
