...
MLflow 是一個開放原始碼平臺，用於管理端對端機器學習生命週期。 MLflow 追蹤是記錄和追蹤定型作業計量、參數和模型成品的元件。
...
既然您已擁有所有必要的資源，您可以執行筆記本來提交作業，該作業會使用 MLflow 來追蹤模型參數、計量和成品。

1. **開啟 Labs/08/使用 MLflow 來追蹤 jobs.ipynb** Notebook。
