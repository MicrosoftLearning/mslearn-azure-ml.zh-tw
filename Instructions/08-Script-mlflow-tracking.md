...
MLflow 是一個開放原始碼平台，可供您管理端對端機器學習生命週期。 MLflow 追蹤是元件，可記錄和追蹤您的定型作業計量、參數和模型成品。

在此練習中，您將使用 MLflow 來追蹤作為命令作業的模型定型執行。
...
1. 開啟 **Labs/08/Use MLflow to track jobs.ipynb** 筆記本。
